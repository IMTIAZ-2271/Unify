...
public class NotificationDAO {
    ...
    // Updates the action state of a notification (1 = accepted/approved, -1 = declined/rejected)
    public void updateNotificationState(int notificationId, int state) throws SQLException {
        try (Connection c = DB.conn(); PreparedStatement ps = c.prepareStatement(
                "UPDATE notifications SET invite_accepted=? WHERE id=?")) {
            ps.setInt(1, state);
