...
public class NotificationsController {
    ...
    @FXML private void initialize() { load(); }

    @FXML private void doMarkAll() {
        try { dao.markAllRead(Session.uid()); load(); } catch (Exception e) { e.printStackTrace(); }
    }

    @FXML private void doRefresh() { load(); }
    ...
    private HBox card(Notification n) {
        ...
        readBtn.setOnAction(e -> { try { dao.markRead(n.getId()); load(); } catch (Exception ex) { ex.printStackTrace(); }});

        Button delBtn = new Button("✕");
        delBtn.getStyleClass().add("btn-danger-small");
        delBtn.setOnAction(e -> { try { dao.delete(n.getId()); load(); } catch (Exception ex) { ex.printStackTrace(); }});
