...
public class AppData {
    ...
    private final List<JoinRequest> sentRequests = new CopyOnWriteArrayList<>();
    ...
    public void loadAll(List<Event> e, List<Group> g,
                        List<Notification> n, List<JoinRequest> jr, List<JoinRequest> sr) {
        events.clear();
        events.addAll(e);
        groups.clear();
        groups.addAll(g);
        ...
        joinRequests.clear();
        joinRequests.addAll(jr);
        sentRequests.clear();
        sentRequests.addAll(sr);
        fireAll();
    }
    ...
    public JoinRequest getSentRequestForGroup(int groupId) {
        if (sentRequests.isEmpty()) return null;
        JoinRequest latestRequest = sentRequests.getFirst();
        for (JoinRequest jr : sentRequests) {
            if (jr.getGroupId() == groupId && jr.getRequestedAt().isAfter(latestRequest.getRequestedAt()))
                latestRequest = jr;
        }
    ...
    public void addSentRequest(JoinRequest jr) {
        sentRequests.add(jr);
    }

    // ── Listener registration ─────────────────────────────────────────────

    public void addEventsListener(Runnable r) {
        onEventsChanged.add(r);
    }

    public void removeEventsListener(Runnable r) {
        onEventsChanged.remove(r);
    }

    public void addGroupsListener(Runnable r) {
        onGroupsChanged.add(r);
    }

    public void removeGroupsListener(Runnable r) {
        onGroupsChanged.remove(r);
    }

    public void addNotifListener(Runnable r) {
        onNotificationsChanged.add(r);
    }

    public void removeNotifListener(Runnable r) {
        onNotificationsChanged.remove(r);
    }
