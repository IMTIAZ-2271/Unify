...
public class GroupDetailController {
    ...
    @FXML
    private VBox groupCalendarContainer;  // for the group calendar tab
    ...
    private HBox memberRow(User u) {
        ...
        if (group.isAdmin() && u.getId() != Session.uid()) {
            ...
            roleBtn.setOnAction(e -> {
                roleBtn.setText(isAdmin ? "Make Admin" : "Remove Admin");
                roleBtn.getStyleClass().setAll(isAdmin ? "btn-primary-small" : "btn-warning-small");
                remBtn.setVisible(isAdmin);
                remBtn.setManaged(isAdmin);
                try {
